...
void vis_keymap_disable(Vis *vis) {
	return vis->ui->disable_keymap(vis->ui);
}

void vis_keymap_enable(Vis *vis) {
	return vis->ui->enable_keymap(vis->ui);
}
...
int vis_run(Vis *vis) {
	...
	while (vis->running) {
		...
		if (!FD_ISSET(vis->ui->input_fd, &fds)) {
			if (vis->mode->idle)
				vis->mode->idle(vis);
			timeout = NULL;
			continue;
		...
		if ((n = read(vis->ui->input_fd, &buf[i], sizeof buf)) > 0) {
			len += n;
		} else if (n == 0) {
