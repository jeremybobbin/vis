...
bool view_coord_get(View *view, size_t pos, Line **retline, int *retrow, int *retcol) {
	...
	if (pos < view->start || (pos >= view->end && view->end != text_size(view->text))) {
...
void view_draw(View *view) {
	...
	while (rem > 0) {
		...
		if (cell.width == 0) {
			prev_cell.len = strlen(prev_cell.data);
			memcpy(prev_cell.data + prev_cell.len, cell.data, (sizeof(prev_cell.data)-1)-prev_cell.len);
